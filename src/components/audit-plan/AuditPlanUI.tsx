...
const API_BASE = "/api/audit-plan/files";
const ROOT_LABEL = "Annual Internal Audit Plan";

export default function AuditPlanUI({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const refresh = async () => {
    try {
      ...
      setLoadError("Failed to load audit plan files");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="mb-6 flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              {isAdmin
                ? "Manage the shared library of annual internal audit plan documents."
                : "Browse and download the latest annual internal audit plan documents."}
            </p>
          </div>
        </div>
...
function FolderBrowser({
  ...
  const handleDelete = async (id: string, displayName: string, isFolder: boolean) => {
    ...
    if (id.startsWith("__permanent__:")) {
      alert("This is a permanent system folder and cannot be deleted.");
      return;
    }
    const msg = isFolder
  ...
  const handleOpenInOffice = async (fileId: string) => {
    ...
    try {
      const res = await fetch(`/api/onedrive/open-url?fileId=${fileId}&type=audit-plan`);
  ...
  const handleDownloadAll = async () => {    setZipping(true);
    try {
      ...
      const filename =
        ...
        `${currentPath[currentPath.length - 1] ?? ROOT_LABEL}.zip`;
