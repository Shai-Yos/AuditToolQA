...
function AuditOwnerSidebarContent({
  collapsed,
  ownedAuditIds,
  pathname,
}: {
  ...
  const isOwnerOfCurrentAudit = activeAudit
    ? ownedAuditIds.includes(activeAudit.id)
    : false;
  ...
  return (
    <AuditSidebarItems
      ...
      isAuditOwner={isOwnerOfCurrentAudit}
    />
  );
}

export default function AuditOwnerShell({
  user,
  children,
  appLogo,
  ownedAuditIds,
  ...
  useEffect(() => {
    const check = async () => {
      ...
      try {
        ...
        if (data.role === "ADMIN") setPromotedRole("Admin");
        if (data.role === "USER") setPromotedRole("User");
      } catch {
    ...
    <AuditNavProvider>
      ...
      {promotedRole && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="mx-4 max-w-md rounded-2xl bg-white dark:bg-slate-800 p-8 shadow-2xl text-center">
            ...
            <p className="mb-6 text-slate-500 dark:text-slate-400">
              Your role has changed to <span className="font-semibold text-slate-700 dark:text-slate-200">{promotedRole}</span>. Please sign in again to access your updated permissions.
            </p>
            <button
      ...
      >
        ...
        >
          ...
          <div className="flex h-16 shrink-0 items-center border-b border-slate-700/60 px-3">
            <div className="flex min-w-0 flex-1 items-center">
              <button onClick={() => setSidebarCollapsed((c) => !c)} className="shrink-0 cursor-pointer" title={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}>
                <img src={appLogo ?? "/favicon.ico"} alt="App Logo" className="h-10 w-10 object-contain" />
              </button>
              {!sidebarCollapsed && (
                <Link href="/auditOwnerDashboard" className="ml-3 flex-1 truncate text-sm font-semibold tracking-tight text-slate-300 hover:text-white transition-colors">
                  Audit Tool
                </Link>
              )}
            </div>
            <button onClick={() => setSidebarOpen(false)} className={`rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-800 hover:text-white lg:hidden ${sidebarCollapsed ? "ml-auto" : "ml-2"}`} aria-label="Close sidebar">
              ✕
            </button>
          </div>
          ...
          <nav className="flex flex-1 flex-col overflow-y-auto px-2 py-4">
            ...
            <NavItem
              href="/auditOwnerDashboard"
              icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>}
              label="Dashboard"
              active={pathname === "/auditOwnerDashboard"}
              collapsed={sidebarCollapsed}
            />
            <NavItem
              href="/auditOwnerDashboard/allAudits"
              icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>}
              label="All Audits"
              active={pathname.startsWith("/auditOwnerDashboard/allAudits")}
              collapsed={sidebarCollapsed}
              newTab
            />
            <NavItem
              href="/auditOwnerDashboard/allRequests"
              ...
              label="All Requests"
              active={pathname.startsWith("/auditOwnerDashboard/allRequests")}
              collapsed={sidebarCollapsed}
              newTab
            />
            <NavItem
              href="/auditOwnerDashboard/createAudit"
              ...
              label="Create Audit"
              active={pathname.startsWith("/auditOwnerDashboard/createAudit")}
              collapsed={sidebarCollapsed}
            />
            <NavItem
              href="/auditOwnerDashboard/auditPlan"
              ...
              active={pathname.startsWith("/auditOwnerDashboard/auditPlan")}
              collapsed={sidebarCollapsed}
              newTab
            />
            <NavItem
              href="/auditOwnerDashboard/riskAssessments"
              icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" /></svg>}
              label="Annual Internal Audit Risk Assessments"
              active={pathname.startsWith("/auditOwnerDashboard/riskAssessments")}
              collapsed={sidebarCollapsed}
              newTab
            />
            <NavItem
              href="/auditOwnerDashboard/sirt"
              icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-4.35-4.35M17 11A6 6 0 105 11a6 6 0 0012 0z" /></svg>}
              label="Site Investigation Response Team (SIRT)"
              active={pathname.startsWith("/auditOwnerDashboard/sirt")}
              collapsed={sidebarCollapsed}
              newTab
            />
            <NavItem
              href="/auditOwnerDashboard/miscellaneous"
              icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 7a2 2 0 012-2h5l2 2h7a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z" /></svg>}
              label="Miscellaneous"
              active={pathname.startsWith("/auditOwnerDashboard/miscellaneous")}
              collapsed={sidebarCollapsed}
              newTab
            />
            ...
            {!sidebarCollapsed && (
              <p className="mb-2 mt-6 px-3 text-[10px] font-semibold uppercase tracking-widest text-slate-500">Management</p>
            )}
            {sidebarCollapsed && <div className="my-3 border-t border-slate-700/60" />}
            <NavItem
              href="/auditOwnerDashboard/my-feedback"
              icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" /></svg>}
              label="My Feedback"
              active={pathname.startsWith("/auditOwnerDashboard/my-feedback")}
              collapsed={sidebarCollapsed}
              newTab
            />
            <NavItem
              href="/auditOwnerDashboard/requestRole"
              icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z" /></svg>}
              label="Request Role Upgrade"
              active={pathname.startsWith("/auditOwnerDashboard/requestRole")}
              collapsed={sidebarCollapsed}
              newTab
            />
          </nav>
          ...
          <div className="shrink-0 border-t border-slate-700/60 px-2 py-3">
            <div className="group relative">
              <Link
                href="/auditOwnerDashboard/profile"
                title={sidebarCollapsed ? "Profile" : undefined}
                className={`flex items-center overflow-hidden rounded-xl px-2.5 py-2.5 text-sm font-medium transition ${sidebarCollapsed ? "justify-center" : "gap-3"} ${pathname.startsWith("/auditOwnerDashboard/profile") ? "bg-slate-800 text-white" : "text-slate-400 hover:bg-slate-800 hover:text-white"}`}
              >
              ...
              {!sidebarCollapsed && (
                <a href="/auditOwnerDashboard/profile" target="_blank" rel="noopener noreferrer" title="Open in new tab" onClick={(e) => e.stopPropagation()} className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-slate-300 opacity-0 transition hover:bg-slate-700 hover:text-white group-hover:opacity-100">
            ...
            <button
              ...
              className={`mt-1 flex w-full items-center rounded-xl px-2.5 py-2.5 text-sm font-medium text-slate-400 transition hover:bg-slate-800 hover:text-white ${sidebarCollapsed ? "justify-center" : "gap-3"}`}
            >
        ...
        <div className="print:hidden sticky top-0 z-10 flex h-14 items-center gap-3 border-b border-slate-700/60 bg-slate-900 px-4 shadow-sm lg:hidden">
          <button onClick={() => setSidebarOpen(true)} className="rounded-lg p-2 text-slate-400 transition hover:bg-slate-800 hover:text-white" aria-label="Open menu">☰</button>
          <img src={appLogo ?? "/favicon.ico"} alt="App Logo" className="h-8 w-8 object-contain" />
