...
const API_BASE = "/api/miscellaneous/files";
const ROOT_TITLE = "Miscellaneous";

export default function MiscellaneousUI({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const refresh = async () => {
    try {
      ...
      setLoadError("Failed to load Miscellaneous files");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="mb-6 flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              {isAdmin
                ? "Manage the shared library of Miscellaneous documents."
                : "Browse and download Miscellaneous documents."}
            </p>
          </div>
        </div>
...
function FolderBrowser({
  ...
  const handleOpenInOffice = async (fileId: string) => {
    ...
    try {
      const res = await fetch(`/api/onedrive/open-url?fileId=${fileId}&type=miscellaneous`);
