...
export default function ChatsUI({
  ...
  const [assignedToAudit, setAssignedToAudit] = useState(isAssignedToAudit);
  const [liveTranscriptionFrIndices, setLiveTranscriptionFrIndices] = useState(transcriptionFrIndices);
  const [liveCommFrIndices, setLiveCommFrIndices] = useState(commFrIndices);
  ...
  useEffect(() => {
    setActiveAudit({ id: auditId, title: liveTitle, tab: "chat", canCreateRequest });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [auditId, liveTitle, canCreateRequest]);
  ...
  // Poll assignment + transcription roles every 5s so changes take effect without a page reload
  useEffect(() => {
    const poll = async () => {
      try {
        const res = await fetch(`/api/audits/${auditId}/assignment`);
        if (!res.ok) return;
        const data = (await res.json()) as { isAssigned: boolean; transcriptionFrIndices: number[]; commFrIndices: number[]; roles: string };
        setAssignedToAudit(data.isAssigned);
        setLiveTranscriptionFrIndices(data.transcriptionFrIndices);
        setLiveCommFrIndices(data.commFrIndices);
            ...
            return (
              <>
                ...
                {ordered.map((ch, idx) => {
                  ...
                  if (commMatch) {
                    const canComm = currentUser.isAdmin || liveCommFrIndices.includes(frNum);
                    ...
                    panel = (
                      <ChatPanel
                        ...
                        readOnly={!assignedToAudit || !canComm}
                        roomUsers={roomUsers}
                        frIndex={frNum}
...
export function ChatPanel({
  ...
  // Auto-resize textarea (chat composer only; notepad uses flex) (chat composer only; notepad uses flex)
  useEffect(() => {
  ...
  // Auto-save for transcription mode every 1.5s (saves while typing)
    ...
    const save = async () => {
      ...
      try {
        if (!v.trim() || isEmptyHtml(v)) {
          for (const oldId of savedMsgIdsRef.current) {
            fetch(`/api/audits/${auditId}/chat`, {
              method: "DELETE",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ messageId: oldId }),
            }).catch(() => {});
          }
          ...
          if (existingId) {
            const res = await fetch(`/api/audits/${auditId}/chat`, {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ messageId: existingId, text: v }),
            });
            if (res.ok) {
            ...
            const res = await fetch(`/api/audits/${auditId}/chat`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ channel, text: v }),
            });
            if (res.ok) {
              const data = (await res.json()) as { ok: boolean; message: Message };
              savedMsgIdsRef.current = [data.message.id];
      ...
      } catch { setSaveStatus("idle"); } finally { savingRef.current = false; }
    };
  ...
  // Live-poll transcription content for read-only viewers (every 2s)
  // This is the ONLY poller for read-only transcription panels (incremental + full sync polls are skipped)
  useEffect(() => {
    if (!rightPanel || !readOnly) return;
    const poll = async () => {
      ...
      try {
        ...
        // Use only the latest message to avoid duplicates during save transitions
  ...
  // Polling — skip for read-only transcription panels (handled by the 5s read-only poll above)
  useEffect(() => {
    if (rightPanel && readOnly) return;
    const poll = async () => {
      if (document.hidden) return;
  ...
  // Skip for read-only transcription panels (the 2s read-only poll already fetches everything)
  useEffect(() => {
    if (rightPanel && readOnly) return;
        ...
        >
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-3">
              <div className="flex h-16 w-16 items-center justify-center rounded-full ring-1 shadow-sm bg-white ring-slate-200">
    ...
    <div className="flex flex-col gap-1.5 max-w-[280px]">
      <a
        ...
        className={[
          ...
          isOwn ? "bg-blue-50 border-blue-200 text-blue-900" : "bg-white border-slate-200 text-slate-800",
          m.isNew ? "animate-[fadeSlideIn_0.3s_ease-out]" : "",
        ].join(" ")}
      >
