...
import AdminAssigneesUI from "@/app/adminDashboard/audits/[auditId]/assignees/ui";
import UserAssigneesUI from "@/app/userDashboard/audits/[auditId]/assignees/ui";

export default async function AuditOwnerAssigneesPage({
  params,
}: {
  ...
  const auditBase = await db.audit.findUnique({
    where: { id: auditId },
    select: { createdById: true },
  });
  if (!auditBase) return notFound();

  const isOwner = auditBase.createdById === currentUser.id;
  ...
  const userInfo = new Map(
    audit.users.map((a) => [
      a.userId,
      { name: a.user?.name ?? a.user?.email ?? a.userId, image: a.user?.image ?? null },
    ]),
  );
  ...
  // Fetch Azure AD profile photos for owner (they get full admin view)
  if (isOwner) {
  ...
  const auditData = { id: audit.id, title: audit.title, users: mergedAssignees, frontRoomsCount: audit.frontRoomsCount };
  const currentUserData = { id: currentUser.id, name: currentUser.name ?? currentUser.email ?? "Audit Owner", isAdmin: isOwner };
  ...
  if (isOwner) {
    return (
      <AdminAssigneesUI
        ...
        editAuditBase="/auditOwnerDashboard"
      />
    );
  }

  const roleString = audit.roomRolesJson
    ? buildUserRolesFromJson(audit.roomRolesJson).get(currentUser.id) ?? ""
    : "";
  const canCreateRequest =
    /\bFR\d+\s+(Lead|QM)\b/i.test(roleString) || /\bBR\d+\s+(Lead|QM)\b/i.test(roleString);
