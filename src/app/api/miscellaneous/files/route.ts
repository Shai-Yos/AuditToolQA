...
const ROOT_LABEL = "Miscellaneous";
...
// GET /api/miscellaneous/files — list from DB (any user)
export async function GET() {
  ...
  const rows = await db.miscellaneousFile.findMany({
...
export async function POST(request: NextRequest) {
  ...
  if (kind === "folder") {
    ...
    const existing = await db.miscellaneousFile.findFirst({
      where: {
    ...
    const created = await db.miscellaneousFile.create({
      data: {
        fileUrl: result.url,
        fileName: folderPath,
      },
...
// DELETE /api/miscellaneous/files?fileId=xxx — delete a file or folder
export async function DELETE(request: NextRequest) {
  ...
  const record = await db.miscellaneousFile.findFirst({
    where: { id: fileId },
  ...
  if (isFolder) {
    ...
    await db.miscellaneousFile.deleteMany({
      where: {
  ...
  await db.miscellaneousFile.deleteMany({ where: { id: fileId } });
