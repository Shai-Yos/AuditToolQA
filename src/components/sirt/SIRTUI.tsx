...
const API_BASE = "/api/sirt/files";
const ROOT_TITLE = "Site Investigation Response Team (SIRT)";

export default function SIRTUI({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const refresh = async () => {
    try {
      ...
      setLoadError("Failed to load SIRT files");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="mb-6 flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              {isAdmin
                ? "Manage the shared library of Site Investigation Response Team documents."
                : "Browse and download Site Investigation Response Team documents."}
            </p>
          </div>
        </div>
...
function FolderBrowser({
  ...
  const handleOpenInOffice = async (fileId: string) => {
    ...
    try {
      const res = await fetch(`/api/onedrive/open-url?fileId=${fileId}&type=sirt`);
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      const filename =
        ...
        `${currentPath[currentPath.length - 1] ?? ROOT_TITLE}.zip`;
