...
export default function RegulatoryImplementationUI({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const refresh = async () => {
    try {
      ...
      setLoadError("Failed to load regulatory implementation files");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="mb-6 flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              {isAdmin
                ? "Manage the shared library of regulatory implementation of lessons learned documents."
                : "Browse and download the latest regulatory implementation of lessons learned documents."}
            </p>
          </div>
        </div>
...
function FolderBrowser({
  ...
  const handleOpenInOffice = async (fileId: string) => {
    ...
    try {
      const res = await fetch(`/api/onedrive/open-url?fileId=${fileId}&type=regulatory-implementation`);
