...
export async function updateRequestAssignees(_: State, input: FormData | UpdateRequestAssigneesInput): Promise<State> {
  ...
  if (addedIds.length > 0) {
    ...
    const assigneeNames = addedUsers.map(u => u.name ?? u.email ?? "Unknown").join(", ");
    await logActivity({
    ...
    const creatorName = currentUser.name ?? currentUser.email ?? "An admin";
  ...
  if (removedIds.length > 0) {
    ...
    const assigneeNames = removedUsers.map(u => u.name ?? u.email ?? "Unknown").join(", ");
    await logActivity({
