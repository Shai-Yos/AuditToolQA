...
export default function KanbanBoardUI({
  ...
  useEffect(() => {
    setActiveAudit({ id: audit.id, title: audit.title, tab: "kanban" });
  ...
  const handleCancelRequest = async (requestId: string, e: React.MouseEvent) => {
    ...
    const cancelledColumn = audit.statusColumns.find((c) => c.name.toLowerCase() === "cancelled");
    if (cancelledColumn) {
      setOptimisticRequests((prev) =>
        prev.map((r) => r.id === requestId ? { ...r, statusColumnId: cancelledColumn.id, statusName: cancelledColumn.name } : r),
      );
    }
    const result = await cancelRequest(requestId, audit.id);
    if (result.ok) {
      router.refresh();
    } else {
      setOptimisticRequests(audit.requests);
  ...
  const handleReworkRequest = async (requestId: string, e: React.MouseEvent) => {
    ...
    if (result.ok) {
      ...
      setOptimisticRequests(audit.requests);
      alert(result.error ?? "Failed to rework request");
    }
  };
              ...
              <div className="flex min-w-max gap-5">
                {audit.statusColumns
                  ...
                  .map((col) => {
                    ...
                    const isCancelledCol = col.name.toLowerCase() === "cancelled";
                    return (
