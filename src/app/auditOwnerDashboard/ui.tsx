...
export default function AuditOwnerDashboardClient({
  user,
  audits,
}: {
  ...
  const filtered = useMemo(() => {
    ...
    return audits.filter((a) => {
      ...
      if (filter === "owned" && !a.isOwned) return false;
    ...
    <div className="relative min-h-screen bg-slate-50">
      ...
      <div className="relative mx-auto w-full max-w-none px-4 py-10 sm:px-6 lg:px-8 xl:px-10">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 p-1">
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-sm font-medium transition ${viewMode === "grid" ? "bg-white text-slate-900 shadow-sm ring-1 ring-slate-200" : "text-slate-500 hover:bg-white hover:text-slate-700"}`}
                  type="button"
                  title="Grid view"
                >
                  ⊞
                ...
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-sm font-medium transition ${viewMode === "list" ? "bg-white text-slate-900 shadow-sm ring-1 ring-slate-200" : "text-slate-500 hover:bg-white hover:text-slate-700"}`}
                  type="button"
                  title="List view"
                >
                  ☰
        ...
        <div className="mt-4 text-sm text-slate-600">
          ...
          {filter === "assigned" ? "assigned" : filter === "owned" ? "owned" : "active"} audits
          {query && (
            <>
              {" "}matching{" "}
            ...
            <div className="col-span-full flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white p-14 text-center shadow-sm">
              ...
              <div className="mt-2 max-w-md text-sm text-slate-600">
                {filter === "assigned" && !query
                  ...
                  : filter === "owned" && !query
                  ? "You do not own any active audits yet."
