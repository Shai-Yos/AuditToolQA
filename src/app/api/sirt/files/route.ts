...
// GET /api/sirt/files — list from DB (any user)
...
export async function POST(request: NextRequest) {
  ...
  if (kind === "folder") {
    ...
    const existing = await db.sirtFile.findFirst({
      where: {
    ...
    const created = await db.sirtFile.create({
      data: {
        fileUrl: result.url,
        fileName: folderPath,
      },
...
// DELETE /api/sirt/files?fileId=xxx — delete a file or folder
export async function DELETE(request: NextRequest) {
  ...
  const record = await db.sirtFile.findFirst({
    where: { id: fileId },
  ...
  await db.sirtFile.deleteMany({ where: { id: fileId } });
