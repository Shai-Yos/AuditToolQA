...
import AllRequestsUserClient from "@/app/userDashboard/audits/[auditId]/requests/ui";

export default async function AuditOwnerRequestsPage({
  params,
}: {
  ...
  const [audit, requests] = await Promise.all([
    db.audit.findUnique({ where: { id: auditId }, select: { title: true, frontRoomsCount: true, roomRolesJson: true, createdById: true } }),
    db.request.findMany({
      where: { auditId },
  ...
  for (const r of requests) {
    ...
    if (!statusMap[r.statusName]) statusMap[r.statusName] = { count: 0, color, order };
    statusMap[r.statusName]!.count += 1;
  }

  const isOwner = audit.createdById === user.id;
  const canCreateRequest = isOwner || (() => {
